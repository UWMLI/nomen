...
(function() {
  'use strict';
  var Dataset, datasetDisplay, readAll, toArray;

  toArray = function(list) {
    return Array.prototype.slice.call(list || [], 0);
  };

  readAll = function(dirReader, action) {
    return dirReader.readEntries((function(_this) {
      return function(results) {
        ...
        if (results.length !== 0) {
          ...
          return readAll(dirReader, action);
        }
      };
    })(this));
  };
  ...
  Dataset = (function() {
    ...
    Dataset.prototype.loadFeatureImages = function(callback) {
      ...
      return resolveLocalFileSystemURL("" + this.dir + "/features/", (function(_this) {
        return function(dirEntry) {
          readAll(dirEntry.createReader(), function(fileEntry) {
            if (fileEntry.isDirectory) {
              return readAll(fileEntry.createReader(), function(image) {
                return _this.addFeatureImage(image);
              });
            }
          });
          return callback();
    ...
    Dataset.prototype.loadSpeciesImages = function(callback) {
      ...
      return resolveLocalFileSystemURL("" + this.dir + "/species/", (function(_this) {
        return function(dirEntry) {
          readAll(dirEntry.createReader(), function(image) {
            return _this.addSpeciesImage(image);
          });
          return callback();
        };
      })(this));
